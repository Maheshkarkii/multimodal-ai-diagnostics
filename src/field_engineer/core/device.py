...
def resolve_device(requested_device: str = "auto") -> torch.device:
    """
    Resolve and validate execution device.
    ...
    logger.info("Execution device resolved to: %s", device)
    return device
...
def set_seed(seed: int = 42, deterministic: bool = True) -> None:
    ...
    Args:
        ...
        deterministic: If True, configures CuDNN backends for reproducibility.
    """
    random.seed(seed)
    np.random.seed(seed)
